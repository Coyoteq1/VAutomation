...
namespace VAutomation.Converters;
...
class FoundMapIconConverter : CommandArgumentConverter<FoundMapIcon>
{
    ...
    static void Initialize()
    {
        ...
        foreach (var entry in CrowbaneArenaCore.SystemService.PrefabCollectionSystem._PrefabGuidToEntityMap)
        {
            var key = entry.Key;
            if (CrowbaneArenaCore.SystemService.PrefabCollectionSystem._PrefabLookupMap.GuidToEntityMap.TryGetValue(key, out var prefab))
            {
                var name = CrowbaneArenaCore.SystemService.PrefabCollectionSystem._PrefabLookupMap.GetName(key);
                if (name.StartsWith("MapIcon_"))
                {
                    if (excludedMapIcons.Contains(name[8..]))
                        continue;

                    if (!VRisingCore.EntityManager.HasComponent<MapIconData>(prefab))
                        continue;
    ...
    public override FoundMapIcon Parse(ICommandContext ctx, string input)
    {
        ...
        {
            if (!CrowbaneArenaCore.SystemService.PrefabCollectionSystem._PrefabLookupMap.TryGetName(new(integral), out var name))
