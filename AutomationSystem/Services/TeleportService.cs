...
public static class TeleportService
{
    ...
    private static float3 GetArenaSpawnFromConfig()
    {
        ...
        {
            var enabledZone = ArenaConfigLoader.ArenaSettings.Zones.FirstOrDefault(z => z.Enabled);
            if (enabledZone != null) return new float3(enabledZone.SpawnX, enabledZone.SpawnY, enabledZone.SpawnZ);
        }
