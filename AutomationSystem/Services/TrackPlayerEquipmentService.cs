...
namespace CrowbaneArena.Services
{
    ...
    {
        public static TrackPlayerEquipmentService Instance { get; private set; }
        ...
        {
            Instance = this;
            // Start monitoring coroutine if Core has StartCoroutine
            if (CrowbaneArenaCore.Instance != null)
            {
                CrowbaneArenaCore.Instance.StartCoroutine(MonitorForEquipmentChanges());
            }
        }
        ...
        public void StartTrackingPlayerForNoDurability(Entity player)
        {
            ...
            Plugin.Logger?.LogInfo($"Started tracking equipment durability for player {player}");
        }
        ...
        public void StopTrackingPlayerForNoDurability(Entity player)
        {
            ...
            Plugin.Logger?.LogInfo($"Stopped tracking equipment durability for player {player}");
        }
        ...
        void RestoreItemDurability(TrackedItem trackedItem)
        {
            ...
            // Try to get original durability settings from prefab
            if (trackedItem.Item.Has<PrefabGUID>())
            {
                var prefabGUID = trackedItem.Item.Read<PrefabGUID>();
                // Note: This requires access to PrefabCollectionSystem which may not be available
                // For now, we'll set reasonable defaults
                durability.LossType = DurabilityLossType.Damage; // Default loss type
                durability.TakeDamageDurabilityLossFactor = 1.0f; // Default damage factor
            }
