...
namespace CrowbaneArena.Services
{
    class PortalService
    {
        ...
        {
            if (!CrowbaneArenaCore.PrefabCollection._SpawnableNameToPrefabGuidDictionary.TryGetValue("TM_General_Entrance_Gate", out var portalPrefabGUID))
                Plugin.Logger?.LogError("Failed to find TM_General_Entrance_Gate PrefabGUID");
            if (!CrowbaneArenaCore.PrefabCollection._PrefabGuidToEntityMap.TryGetValue(portalPrefabGUID, out portalPrefab))
                Plugin.Logger?.LogError("Failed to find TM_General_Entrance_Gate Prefab entity");
            ...
            spawnedPortalQuery = CrowbaneArenaCore.EntityManager.CreateEntityQuery(ref queryBuilder);
        ...
        public string EndPortal(Entity playerEntity, PrefabGUID endMapIcon)
        {
            ...
            Plugin.Logger?.LogInfo($"Start: {start.chunk} {start.index} End: {endChunk} {endIndex}");
            return null;
        }
        ...
        {
            var startPortal = CrowbaneArenaCore.EntityManager.Instantiate(portalPrefab);
            ...
            var attachMapIconsToEntity = CrowbaneArenaCore.EntityManager.GetBuffer<AttachMapIconsToEntity>(startPortal);
        ...
        {
            if (!CrowbaneArenaCore.ChunkObjectManager._ChunkPortals.TryGetValue(chunk, out var portalList))
                return 0;
        ...
        {
            if (!CrowbaneArenaCore.ChunkObjectManager._ChunkPortals.TryGetValue(chunkPortal.ToChunk, out var portalDataList))
                return Entity.Null;
        ...
        {
            if (!CrowbaneArenaCore.ChunkObjectManager._ChunkPortals.TryGetValue(chunkPortal.FromChunk, out var portalDataList))
                return;
            portalDataList[chunkPortal.FromChunkPortalIndex] = new();
            CrowbaneArenaCore.ChunkObjectManager._ChunkPortals.Remove(chunkPortal.FromChunk);
            CrowbaneArenaCore.ChunkObjectManager._ChunkPortals.Add(chunkPortal.FromChunk, portalDataList);
        }
        ...
        public bool DestroyPortal(Entity senderCharacterEntity)
        {
            ...
            {
                var attachedBuffer = CrowbaneArenaCore.EntityManager.GetBuffer<AttachedBuffer>(closestPortal);
            ...
            CrowbaneArenaCore.EntityManager.DestroyEntity(closestPortal);
            ...
            if (connectedPortal != Entity.Null)
            {
                ...
                {
                    var attachedBuffer = CrowbaneArenaCore.EntityManager.GetBuffer<AttachedBuffer>(connectedPortal);
                    for (var i = 0; i < attachedBuffer.Length; i++)
                    {
                        var attachedEntity = attachedBuffer[i].Entity;
                        if (attachedEntity == Entity.Null) continue;
                ...
                CrowbaneArenaCore.EntityManager.DestroyEntity(connectedPortal);
            }
            return true;
        }
    }
